...
export default function AvailabilityEditor({ initial }: { initial: Availability[] }) {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-lg font-semibold text-slate-100">My availability</h1>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 space-y-3">
        <div className="flex gap-2">
          <button onClick={() => setTab("weekly")} className={`px-3 py-1 text-sm rounded-md ${tab === "weekly" ? "bg-slate-100 text-slate-900" : "text-slate-300 border border-slate-700"}`}>Weekly</button>
          <button onClick={() => setTab("date")} className={`px-3 py-1 text-sm rounded-md ${tab === "date" ? "bg-slate-100 text-slate-900" : "text-slate-300 border border-slate-700"}`}>Specific date</button>
        </div>

        <div className="grid grid-cols-2 gap-3">
          {tab === "weekly" ? (
            <label className="text-xs text-slate-400 col-span-2">Day
              <select value={weekday} onChange={(e) => setWeekday(Number(e.target.value))} className="mt-1 w-full bg-slate-800 border border-slate-700 rounded-md px-2 py-2 text-slate-100">
                {DAYS.map((d, i) => <option key={i} value={i}>{d}</option>)}
              </select>
            </label>
          ) : (
            <label className="text-xs text-slate-400 col-span-2">Date
              <input type="date" value={date} onChange={(e) => setDate(e.target.value)} className="mt-1 w-full bg-slate-800 border border-slate-700 rounded-md px-2 py-2 text-slate-100" />
            </label>
          )}
          <label className="text-xs text-slate-400">From
            <input type="time" value={start} onChange={(e) => setStart(e.target.value)} className="mt-1 w-full bg-slate-800 border border-slate-700 rounded-md px-2 py-2 text-slate-100" />
          </label>
          <label className="text-xs text-slate-400">To
            <input type="time" value={end} onChange={(e) => setEnd(e.target.value)} className="mt-1 w-full bg-slate-800 border border-slate-700 rounded-md px-2 py-2 text-slate-100" />
          </label>
        </div>
        ...
        <input value={note} onChange={(e) => setNote(e.target.value)} placeholder="Note (optional)" className="w-full bg-slate-800 border border-slate-700 rounded-md px-2 py-2 text-slate-100 text-sm" />
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        <div className="text-sm font-medium text-slate-300 mb-3">Weekly</div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
        <div className="text-sm font-medium text-slate-300 mb-3">Specific dates</div>
...
function Row({ label, a, onDel }: { label: string; a: Availability; onDel: (id: number) => void }) {
  return (
    <div className="flex items-center justify-between text-sm border-b border-slate-800 pb-2 last:border-0">
      <div>
